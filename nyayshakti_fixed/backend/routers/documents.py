...
class TemplateRequest(BaseModel):
    template_id: str
    data: dict
    images: list = []   # base64 encoded evidence/proof images
...
async def generate_template(req: TemplateRequest):
    try:
        ...
        gen_map = {
            ...
            "fir_draft":        lambda d, images=None: __import__("utils.fir_generator",
                                    fromlist=["generate_fir_document"]).generate_fir_document(d, images or []),
        }
        ...
        pdf_bytes = fn(req.data, images=req.images or [])
        if not pdf_bytes:
