...
async def generate_action_plan(req: FlowRequest):
    ...
    Returns a safe fallback (never raises 500) so the frontend always gets a usable response.
    """
    try:
        flow = FLOWS.get(req.flow_id)
        if not flow:
            return {"plan": "⚠️ Flow not found. Please try again or use the Chat feature.", "flow_title": ""}
    except Exception:
        return {"plan": "⚠️ An error occurred. Please use the Chat feature or call NALSA: 1800-110-370.", "flow_title": ""}
...
- Respond entirely in {lang_name}"""
    ...
    if not GEMINI_AVAILABLE or client is None:
        return {
            "plan": (
                f"## ⚖️ {flow['title']}\n\n"
                "**AI service is currently unavailable.** Here is what you can do right now:\n\n"
                "### 📞 Free Legal Help (Call Now)\n"
                "- **NALSA Helpline:** 1800-110-370 (toll-free, 24x7)\n"
                "- **Tele-Law:** 1800-120-1075\n"
                "- **Police:** 100 | **Women Helpline:** 1091 | **Legal Aid:** 15100\n\n"
                "### 🏛️ Visit Nearest Office\n"
                "1. **District Legal Services Authority (DLSA)** — free legal aid for all\n"
                "2. **District Court** — file FIR, complaints, petitions\n"
                "3. **Consumer Forum** — for consumer and service complaints\n\n"
                "### 📋 General Steps\n"
                "1. Write down everything that happened with dates and names\n"
                "2. Collect and keep all documents, messages, receipts\n"
                "3. Send a written complaint to the other party first\n"
                "4. Approach the relevant authority or court with your documents\n\n"
                "_Use the **Chat** feature above to ask specific legal questions._"
            ),
    ...
    return {
        "plan": (
            f"## ⚖️ {flow['title']}\n\n"
            "**Sorry, the AI plan could not be generated right now.** Here is immediate help:\n\n"
            "### 📞 Call Free Legal Help\n"
            "- **NALSA:** 1800-110-370 (toll-free)\n"
            "- **Tele-Law:** 1800-120-1075\n\n"
            "### 💬 Try the Chat Feature\n"
            "Close this panel and use the **Chat** button — describe your situation and our AI will guide you.\n\n"
            "### 🏛️ Offline Help\n"
