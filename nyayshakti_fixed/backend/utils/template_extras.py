...
def _si(value, default=0) -> int:
    """Safely convert form value (str/None) to int."""
    try:
...
def _doc(filename, story, images=None):
    doc = SimpleDocTemplate(filename, pagesize=A4,
        rightMargin=0.7*inch, leftMargin=0.7*inch,
        topMargin=0.6*inch, bottomMargin=0.6*inch)
    if images:
        _do_embed_images(story, images, getSampleStyleSheet())
    doc.build(story)
    with open(filename, "rb") as f: b = f.read()
    os.unlink(filename)
    return b

def _font(lang='English'):
    try:
        from utils.font_setup import get_font_for_lang
        return get_font_for_lang(lang)
    except Exception:
        return 'Helvetica'
...
def _styles(lang='English'):
    ...
    h2     = ParagraphStyle("H2", parent=s["Heading2"], fontSize=12, fontName=hb,
...
def _do_embed_images(story, images, styles):
    """Embed base64 images as exhibit pages."""
    if not images:
        return
    ...
    try:
        ...
        story.append(Paragraph("[Images could not be embedded: Pillow not installed]", styles["Normal"]))
        return
    story.append(Spacer(1, 0.3*inch))
    story.append(Paragraph("<b>ATTACHED EVIDENCE</b>", styles["Heading3"]))
    for idx, img_b64 in enumerate(images, 1):
        try:
            ...
            story.append(Paragraph(f"[Exhibit {idx}: could not embed — {e}]", styles["Normal"]))

def generate_rental_agreement(data: dict, images: list = None) -> bytes:
    lang = data.get("_lang", "English") if isinstance(data, dict) else "English"
    ...
    return _doc(fn, story, images)


# ── Sale Agreement ────────────────────────────────────────────────────────────
def generate_sale_agreement(data: dict, images: list = None) -> bytes:
    lang = data.get("_lang", "English") if isinstance(data, dict) else "English"
    fn = tempfile.mktemp(suffix=".pdf")
    title, sub, h2, body, clause = _styles()
    story = []
...
def generate_employment_offer(data: dict, images: list = None) -> bytes:
    lang = data.get("_lang", "English") if isinstance(data, dict) else "English"
    fn = tempfile.mktemp(suffix=".pdf")
    title, sub, h2, body, clause = _styles()
    story = []
