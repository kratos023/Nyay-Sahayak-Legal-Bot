...
export default function LegalNews({ onClose, language, onSendToChat }: Props) {
  ...
  async function loadNews() {
    ...
    try {
      ...
      if (fetched.length === 0) {
        setError("No articles found right now. Legal news feeds may be temporarily unavailable — try again in a few minutes.");
      }
      setArticles(fetched);
    } catch (e: any) {
      if (e?.name === "TimeoutError") {
        setError("Request timed out. The news feeds are slow right now — please try again.");
      } else {
        setError("Could not load news. Please check your internet connection and that the backend is running.");
      }
    } finally {
      setLoading(false);
    }
