...
export default function SignupPage() {
  ...
  return (
    <div className="h-full flex items-center justify-center relative overflow-hidden px-4" style={{ background:"var(--bg)" }} suppressHydrationWarning>
      ...
      <div className="relative w-full max-w-sm space-y-8 animate-fade-up">
        <div className="text-center space-y-3">
          <div className="relative inline-block">
            ...
            <div className="relative w-14 h-14 mx-auto rounded-2xl flex items-center justify-center"
              style={{ background:"linear-gradient(135deg,#b45309,#78350f)", border:"1px solid rgba(201,146,10,0.4)" }}>
              <Scale size={26} className="text-amber-100"/>
            </div>
          </div>
          <div suppressHydrationWarning>
            <h1 className="font-display text-2xl" style={{ background:"linear-gradient(90deg,#f0b429,#c9920a)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>
              Nyay-Sahayak
            </h1>
            <p className="text-sm mt-1" style={{ color:"var(--text-3)" }} suppressHydrationWarning>{t.createAccountSubtitle}</p>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <input type="text" required value={name} onChange={e=>setName(e.target.value)}
                placeholder="Deepanshu Sharma" className={inputCls} style={iStyle} />
            </div>
            <div className="space-y-1.5">
              <label className="text-[0.65rem] uppercase tracking-widest block" style={{ color:"var(--text-3)" }} suppressHydrationWarning>
                {t.emailLabel}
            ...
            <div className="space-y-1.5">
              <label className="text-[0.65rem] uppercase tracking-widest block" style={{ color:"var(--text-3)" }} suppressHydrationWarning>
                {t.passwordLabel} <span className="normal-case text-[0.6rem]" style={{ color:"var(--text-3)" }}>({t.passwordHint})</span>
              </label>
              <div className="relative">
                <input type={showPw?"text":"password"} required value={password} minLength={6}
            ...
            <button type="submit" disabled={loading}
              className="w-full py-3 rounded-xl font-semibold text-sm disabled:opacity-50 transition-all active:scale-[0.98] flex items-center justify-center gap-2 mt-2"
        ...
        <p className="text-center text-sm" style={{ color:"var(--text-3)" }} suppressHydrationWarning>
          {t.alreadyHaveAccount}{" "}
          <Link href="/login" className="font-medium transition-colors" style={{ color:"var(--gold-light)" }}>
            {t.signIn}
          </Link>
        </p>
      </div>
