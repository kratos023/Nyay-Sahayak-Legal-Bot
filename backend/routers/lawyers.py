...
async def search_lawyers(req: LawyerSearchRequest):
    ...
    # Generate search links — only working, useful ones
    search_links = [
        {
            "label": f"🗺️ Google Maps — {spec_label} Lawyers near {req.location}",
            "url":   f"https://www.google.com/maps/search/{quote_plus(spec_label + ' advocate lawyer near ' + req.location)}",
            "desc":  "Find nearby lawyers with ratings, reviews and directions",
        },
        {
            "label": f"🔍 Google Search — {spec_label} Advocates in {req.location}",
            "url":   f"https://www.google.com/search?q={quote_plus(spec_label + ' advocate lawyer ' + req.location)}",
            "desc":  "Search for advocates, law firms and consultations",
        },
    ]
    ...
    if req.language_pref and req.language_pref != "English":
        search_links.append({
            "label": f"🗣️ {req.language_pref}-speaking {spec_label} Lawyers",
            "url":   f"https://www.google.com/search?q={quote_plus(req.language_pref + ' speaking ' + spec_label + ' advocate ' + req.location)}",
            "desc":  f"Find lawyers who speak {req.language_pref} in your area",
        })
